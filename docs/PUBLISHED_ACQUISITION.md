...
forms for the guard; the collector also checks percent and form encoding.
...
Before committing a source, retain the original pages, the declaration and the
receipt beside its complete licence/provenance record. Name the receipt
`<artifact>.acquisition.json` beside `<artifact>.provenance.json`; record the
derived source path/hash at `artifacts.runtimeJsonl` in provenance. The receipt
is acquisition evidence, not a substitute for provider, origin, licence,
attribution and reproducible derivation instructions.

`verifyPublishedAcquisitions` scans committed provenance records and requires
each adjacent scope declaration. Bounded-window admission reuses the original
FSC derivation and checks source/generated bytes against recorded hashes.
