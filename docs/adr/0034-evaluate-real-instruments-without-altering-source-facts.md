...
The first reading is the repository's implemented boundary. The cross-market
session-reversal engine evaluates fixed, licensed KOSPI 200 index and futures
rows and pins a `SUPPORTED` result. The source rows remain unchanged, while a
separate case manifest declares the scope and thresholds and a versioned rule
computes the result. Reading the provenance rule as a ban on that evaluation
...
contract and report `SUPPORTED`, `NOT_SUPPORTED` or `INCONCLUSIVE` for a
declared pattern hypothesis only when all of these conditions hold:
...
The committed cross-market golden and published case remain permitted. Their
result is evidence that one versioned rule was satisfied under one approved
scope, not an attribute of the KOSPI 200 or its futures contract and not a
general statement about the market.
