...
const LAYERS_KO: readonly (readonly [string, string])[] = [
  [
    "L1 · 해석",
    "제약된 매퍼가 대상 필드와 변환, 확신도, 근거를 제안합니다. 기본값은 결정론적 fixture입니다. 설정을 명시하면 합성 방언 두 개에 한해 매핑을 받아올 수 있고, 전송 검사는 아직 모킹입니다. 행을 고치거나 결과를 정하지는 못합니다.",
  ],
  [
    "L2 · 승인",
    "사람이 매핑과 사례 제안을 승인합니다. 승인은 그 제안의 해시와 정확히 묶입니다. 표시된 필드는 사유를 적은 override 없이 넘어가지 못합니다. 승인으로 계산된 결과를 고칠 수는 없습니다.",
  ],
  [
    "L3 · 판정",
  ...
  [
    "L4 · 증거",
    "서버가 발견을 정본 eventId와 rawRowHash, 아티팩트 좌표, 그리고 손대지 않은 원본 값까지 되짚습니다. 되짚지 못하는 계보는 거부합니다. INCONCLUSIVE에는 발견 증거가 없습니다.",
  ],
];
...
export const architectureCopy: Readonly<Record<Language, ArchitectureCopy>> = {
  ...
  ko: {
    ...
    readingLede:
      "다이어그램은 계층 모델을 보여줍니다. 어떤 구성요소가 구현됐고 어떤 것이 아직 계획인지는 아래 체인에 표시했습니다.",
    authorityTitle: "모델의 권한이 끝나는 지점.",
    authorityText:
      " L1은 제안합니다. 지금은 필드 매핑이고, 아래의 한정된 사례 제안기는 계획 단계입니다. 값을 계산하거나 행을 고치거나 결과를 정하는 일은 하지 않습니다.",
    boundaryTitle: "신뢰 경계.",
    boundaryTextBefore:
      " 경계는 L2와 L3 사이에 있습니다. 승인이 제안 해시와 정확히 묶이지 않으면 아무것도 결정론적 core에 닿지 못합니다. 검증이나 승인 게이트를 충족하지 못하면 결과 대신 검토 상태가 돌아옵니다. 엔진의 다섯 규칙 gate는 경계 안쪽에 있고, 그중 하나가 통과하지 못하면 ",
    ...
    canonicalOne:
      "는 엔진 버전과 의미 이벤트 투영을 해시하고, 평가가 있으면 그 평가까지 해시합니다. 실행할 때마다 달라지는 메타데이터는 빠집니다.",
    canonicalTwo:
      "이 결과 해시는 승인 기록 전부와 매핑·manifest의 모든 필드, 소스 추적까지 덮지는 않습니다. 번들 해시와 독립적인 번들 조립·검증은 아직 계획입니다.",
    canonicalThree:
      "브라우저에 남는 가이드 진행 상태는 화면 표시용입니다. API 리플레이는 요청마다 자기 워크플로를 새로 만들고, 계속 남는 감사 이력은 아직 없습니다.",
    walkThrough: "사례 따라가기",
  },
};
