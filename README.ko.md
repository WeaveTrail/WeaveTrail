...
![공개된 기록으로 하루를 다시 계산한 사례. 지수와 선물의 발행된 그날 값, 실행 전에 사람이 확정하는 범위, 그리고 고정된 코드가 내놓은 값이 나란히 있고, 모든 숫자는 그 값이 나온 원본 행까지 열어 볼 수 있다](docs/assets/worked-case.ko.svg)

공개된 기록은 발행된 그대로 읽습니다. 무엇을 볼지 — 분석할 날, 비교할 기간,
되돌림을 얼마나 크게 볼지 — 는 사람이 실행 전에 정합니다. 그다음 고정된 코드가
계산하고, 그날이 그 기간 안에서 어디쯤인지 알려 줍니다. 숫자마다 그 값이 나온
행을 열어 볼 수 있고, 같은 입력으로 다시 실행하면 같은 결과가 나옵니다.
...
위치가 아니라 권한으로 나눕니다. 각 층은 무엇을 할 수 있는지, 무엇을 절대 할 수
없는지, 무엇을 기록으로 남기는지를 가집니다.

먼저 답하는 질문은 일부러 좁게 잡았습니다.

> 승인된 계좌 집단의 짧은 매수 집중이 가격을 끌어올렸는가, 그리고 그 거래를
> 빼면 그 움직임이 얼마나 남는가?
...
- **해시는 실행이 아니라 답에 대한 것입니다 ·** 같은 행의 순서를 섞어도 해시는
  그대로지만, 승인한 내용을 바꾸면 해시가 바뀌고 그 사실이 기록에 남습니다.
- **거부도 결과입니다 ·** 충족할 수 없는 조건을 만나면 더 약한 답을 내놓는 대신
  멈추고 아무 답도 내놓지 않습니다.
...
[Apache License 2.0](LICENSE)을 따릅니다. 서드파티 패키지는 각자의 라이선스를
유지합니다. [라이선스와 배포](docs/LICENSING.md)(영문)와
