...
![알림은 후보를 지목하지만, 그 체결 내역은 필드 이름도 시각 표기도 다른 두 방언으로 도착한다. 제약된 모델이 둘을 어떻게 맞출지 제안할 수는 있어도, 알림 자체는 어떤 행에서 어떤 매핑과 어느 규칙 버전으로 그 숫자가 나왔는지 남기지 않는다](docs/assets/problem.ko.svg)
...
![시장감시 알림과 다시 계산할 수 있는 결과 사이의 네 층위. 제약된 매퍼가 필드 매핑을 제안하고, 검토자가 그 제안을 아티팩트 해시에 묶어 그대로 승인하고, 버전이 고정된 코드가 판정하고, 증거 층이 모든 발견을 원본 행까지 되짚는다](docs/assets/how-it-works.ko.svg)
