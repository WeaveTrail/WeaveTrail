...
![공개된 기록으로 하루를 다시 계산한 사례. 지수와 선물의 발행된 그날 값, 실행 전에 사람이 확정하는 분석 대상일과 기준선 기간, 그리고 버전이 고정된 규칙이 돌려준 값이 나란히 있다. 관측값마다 견준 기준값이 옆에 적혀 있고, 그 기준값은 이 날의 값을 이미 본 사람이 정했다고 밝혀 둔다](docs/assets/worked-case.ko.svg)
