...
![공식 발표, 공개 시장 데이터, 붙여 넣은 분석 글과 화면의 문장 사이에 계획된 네 층위. 모델이 발표의 사실과 그 원문 구간, 붙여 넣은 글의 주장을 제안하고, 사람이 무엇을 볼지 정하고 브리프에 담을 결론을 채택하고, 고정된 코드가 모든 인용을 원문과 대조하고 검증된 원자료로 모든 숫자를 다시 계산하며, 계획된 화면의 각 문장은 근거까지 열린다. 그 아래에서 원문 인용, 계산 확인, 불일치, 확인 불가는 코드가 확인하고, AI 해석은 모델의 제안이다](docs/assets/layer-separation.ko.svg)
