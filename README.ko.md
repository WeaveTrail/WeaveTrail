...
![공식 발표, 공개 시장 데이터, 붙여 넣은 분석 글과 화면의 문장 사이의 네 층위. 모델이 발표의 사실과 그 원문 구간, 붙여 넣은 글의 주장을 제안하고, 사람이 무엇을 볼지 정하고 브리프에 담을 결론을 채택하고, 고정된 코드가 모든 인용을 원문과 대조하고 모든 숫자를 다시 계산하며, 모든 문장은 근거까지 열린다. 그 아래에서 원문 인용, 계산 확인, 불일치, 확인 불가는 코드가 확인하고, AI 해석은 모델의 제안이다](docs/assets/layer-separation.ko.svg)
...
- **판정 · 고정된 코드 ·** 모든 인용을 원문 바이트와 대조하고, 모든 숫자를 공개
  데이터로 다시 계산합니다. 데이터가 없으면 추측하지 않고 없다고 말합니다.
...
| 계산 확인 | 공개 데이터로 다시 계산한 값이거나, 계산값과 같습니다           | 코드                     |
| 불일치    | 계산하면 다른 값이 나옵니다. 그 값을 옆에 적습니다              | 코드                     |
| 확인 불가 | 공개 자료로는 확인할 수 없습니다. 이유와 필요한 자료를 적습니다 | 코드                     |
