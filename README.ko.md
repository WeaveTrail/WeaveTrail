...
WeaveTrail은 그 둘을 한자리에 모으는 방향으로 개발 중입니다. 공식 발표와 그 뒤의
공개 시장 데이터를 모아 발표마다 사건으로 정리하고, 그 데이터로 말할 수 있는 결론을
한 화면 맨 위에 두는 것이 목표입니다.

**현재 상태:** Evidence Grade `1.0` 계약, 결정론적 검증 경계, 한국어·영어 배지와 tally
컴포넌트는 구현되어 있습니다. 아직 어떤 공개 경로도 이 배지나 tally를 렌더링하지
않으며, 검증된 선언을 화면의 각 문장에 연결하는 작업은 계획 상태입니다. 현재 페이지를
문장별 근거 등급이 적용된 출력으로 읽어서는 안 됩니다.
...
- **제공할 예정인 것 ·** 맨 위의 결론, 문장마다 붙은 근거, 그리고 같은 숫자를 다시
  여는 링크가 달린 한 장 브리프.
...
정해져 있습니다. 계획된 공개 화면에서는 이 분리가 문장마다 붙는 근거 배지로
드러나지만, 현재 경로에는 아직 렌더링되지 않습니다.

![공식 발표, 공개 시장 데이터, 붙여 넣은 분석 글과 화면의 문장 사이에 계획된 네 층위. 모델이 발표의 사실과 그 원문 구간, 붙여 넣은 글의 주장을 제안하고, 사람이 무엇을 볼지 정하고 브리프에 담을 결론을 채택하고, 고정된 코드가 숫자를 포함한 인용은 원문 바이트와 대조하고 계산 주장의 값은 검증된 원자료로 다시 계산하며, 계획된 화면의 각 문장은 근거까지 열린다. 그 아래에서 원문 인용, 계산 확인, 불일치, 확인 불가는 코드가 확인하고, AI 해석은 모델의 제안이다](docs/assets/layer-separation.ko.svg)
...
- **판정 · 고정된 코드 ·** 숫자를 포함한 인용은 보관된 원문 바이트와 대조합니다.
  계산 주장에는 검증된 원자료로 값을 다시 계산합니다. 데이터가 없으면 추측하지
  않고 없다고 말합니다. 출처의 단계는 별도로 기록합니다.
- **증거 ·** 계획된 화면에서는 모든 문장을 원문 구간이나, 그 뒤의 원본
  행·계산식·정의까지 열어 볼 수 있습니다. 출처를 되짚을 수 없는 숫자는 보여 주지
  않고 보류합니다.
